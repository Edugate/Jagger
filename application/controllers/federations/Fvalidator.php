...
class Fvalidator extends MY_Controller {
    ...
    public function detailjson()
    {
       ...
       if(!$loggedin)
       {
          show_error('not authenticated',401);
       }
       ...
       foreach($validators as $v)
       {
          ...
          if($venabled)
          {
             $validator[] = array('id'=>$v->getId(),'fedid'=>$fedid,'name'=>htmlspecialchars($v->getName()),'desc'=>htmlspecialchars($v->getDescription()));
          }
       }
       if(isset($validator['0']))
       {
          echo json_encode($validator['0']);
       }
       else
       { 
           show_error('fed not found',404);
