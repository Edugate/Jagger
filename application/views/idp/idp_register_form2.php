...
echo '<li>' . form_label(lang('rr_descriptivename'), 'deschomeorg');
echo form_input(array(
    'id' => 'deschomeorg',
    'name' => 'deschomeorg',
...
$ssobindprotocols = array('urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST','urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST-SimpleSign','urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect');
...
foreach($ssobindprotocols as $s)
{
   echo '<li class="ssourls">';
   echo form_label(lang('rr_singlesignon_fieldset').'<br /><small>'.$s.'</small>','sso['.$s.']');
   echo form_input(array(
        'id'=>'sso['.$s.']',
        'name'=>'sso['.$s.']',
        'value'=>set_value('sso['.$s.']'),
      ));
   echo '</li>';
}
