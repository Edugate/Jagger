...
| `php-fpm/jagger.pool.conf` | PHP-FPM pool running as the `jagger` user, `open_basedir`-confined to `/opt/jagger:/etc/jagger:/var/log/jagger:/var/lib/jagger:/tmp`, with `session.save_path` pinned to `/var/lib/jagger/sessions` (created by `provision.sh`) rather than relying on the distro's php.ini default -- see the QA pass note in `docs/AUDIT.md` §10. |
